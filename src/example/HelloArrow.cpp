...
int main(int argc, char** argv) {
  ...
  std::unique_ptr<parquet::Hasher> murmurHash3 =
      std::make_unique<parquet::MurmurHash3>();
  murmurHash3->Hash(123);
  std::cout << "Hello arrow/parquet magic number " << magic
            << " BlockSplitBloomFilter hash value is "
            << blockSplitBloomFilter->Hash(magic) << " MurmurHash3 value is "
            << murmurHash3->Hash(magic) << std::endl;
  return 0;
}
