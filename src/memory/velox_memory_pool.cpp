...
public:
    VeloxMemoryAllocatorVariant(MemoryAllocator* petrel_alloc)
            : petrel_alloc_(petrel_alloc) {}
    ...
    void* alloc(int64_t size) {
        void* out;
        if (!petrel_alloc_->Allocate(size, &out)) {
            VELOX_FAIL(
    ...
    void* allocZeroFilled(int64_t numMembers, int64_t sizeEach) {
        void* out;
        if (!petrel_alloc_->AllocateZeroFilled(numMembers, sizeEach, &out)) {
            VELOX_FAIL(
    ...
    void* allocAligned(uint16_t alignment, int64_t size) {
        void* out;
        if (!petrel_alloc_->AllocateAligned(alignment, size, &out)) {
            VELOX_FAIL(
    ...
    void* realloc(void* p, int64_t size, int64_t newSize) {
        void* out;
        if (!petrel_alloc_->Reallocate(p, size, newSize, &out)) {
            VELOX_FAIL(
    ...
    reallocAligned(void* p, uint16_t alignment, int64_t size, int64_t newSize) {
        void* out;
        if (!petrel_alloc_->ReallocateAligned(p, alignment, size, newSize, &out)) {
            VELOX_FAIL(
